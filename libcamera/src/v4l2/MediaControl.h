...
class MediaControl {
 public:
    /**
     * \brief Create the singleton instance of MediaControl
     */
    static void createInstance(libcamera::MediaDevice* isysDev);
 ...
 private:
    ...
    MediaControl(libcamera::MediaDevice* isysDev);
    ~MediaControl();

    static MediaControl* getMediaControlInstance();

    // get entity info.
    libcamera::MediaEntity* getEntityById(uint32_t id);
    libcamera::MediaEntity* getEntityByName(const char* name);

    int setupLink(McLink &link);
    int setRoute(int cameraId, McRoute* route);
    ...
    libcamera::MediaDevice* mIsysDev;

    static MediaControl* sMediaControl;
    static Mutex sLock;
};

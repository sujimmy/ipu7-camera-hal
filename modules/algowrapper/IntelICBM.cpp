...
void* IntelICBM::allocBuffer(uint32_t bufSize, int id) {
    void* ptr = nullptr;
    int ret = posix_memalign(&ptr, PAGE_SIZE_U, PAGE_ALIGN(bufSize));
    CheckAndLogError(ret, nullptr, "@%s, Failed to alloc buffer", __func__);
    mClientBufMems.push_back(ptr);

    return ptr;
}

void IntelICBM::freeAllBufs() {
    for (auto& buf : mClientBufMems) {
        if (buf) free(buf);
    }
}

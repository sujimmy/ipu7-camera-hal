...
StaticGraphStatus GraphResolutionConfigurator::getSensorRoi(const RegionOfInterest& userRoi, SensorRoi& sensorRoi)
{
    ...
    uint32_t inputWidth = static_cast<uint32_t>(outputRunKernel->resolution_history->input_width * _sensorHorizontalScaling);
    uint32_t inputHeight = static_cast<uint32_t>(outputRunKernel->resolution_history->input_height * _sensorVerticalScaling);
    ...
    StaticGraphKernelResCrop* originalScalerCrop)
{
    ...
    runKernel->resolution_info->input_crop.right = GRA_ROUND_UP(roi.cropRight - originalScalerCrop->right, 2);
    runKernel->resolution_info->input_crop.top = GRA_ROUND_DOWN(roi.cropTop - originalScalerCrop->top, 2);
    runKernel->resolution_info->input_crop.bottom = GRA_ROUND_UP(roi.cropBottom - originalScalerCrop->bottom, 2);

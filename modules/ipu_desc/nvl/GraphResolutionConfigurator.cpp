...
GraphResolutionConfigurator::GraphResolutionConfigurator(IStaticGraphConfig* staticGraph) {
    ...
    if (initRunKernelCoord(GraphResolutionConfiguratorKernelRole::DownScaler,
                           _downscalerRunKernelCoord) != StaticGraphStatus::SG_OK ||
        initRunKernelCoord(GraphResolutionConfiguratorKernelRole::UpScaler,
                           _upscalerRunKernelCoord) != StaticGraphStatus::SG_OK ||
        initRunKernelCoord(GraphResolutionConfiguratorKernelRole::EspaCropper,
                           _cropperRunKernelCoord) != StaticGraphStatus::SG_OK ||
    ...
    _heightIn2OutScale =
        static_cast<double>(outputRunKernel->resolution_history->input_height -
                            outputRunKernel->resolution_history->input_crop.top -
                            outputRunKernel->resolution_history->input_crop.bottom) /
        outputRunKernel->resolution_history->output_height;

    _originalCropInputToScaler = cropperRunKernel->resolution_history->input_crop;

    _originalCropScalerToOutput.left =
        outputRunKernel->resolution_history->input_crop.left - _originalCropInputToScaler.left;
    _originalCropScalerToOutput.right =
        outputRunKernel->resolution_history->input_crop.right - _originalCropInputToScaler.right;
    _originalCropScalerToOutput.top =
        outputRunKernel->resolution_history->input_crop.top - _originalCropInputToScaler.top;
    _originalCropScalerToOutput.bottom =
        outputRunKernel->resolution_history->input_crop.bottom - _originalCropInputToScaler.bottom;
    ...
    if (outputRunKernel->resolution_info != nullptr) {
        // First add the crop, translated to sensor units, w/out this kernel's scaling since input
        // crop is done before scaling.
        _originalCropScalerToOutput.left += static_cast<int32_t>(
            outputRunKernel->resolution_info->input_crop.left * _widthIn2OutScale);
        _originalCropScalerToOutput.right += static_cast<int32_t>(
            outputRunKernel->resolution_info->input_crop.right * _widthIn2OutScale);
        _originalCropScalerToOutput.top += static_cast<int32_t>(
            outputRunKernel->resolution_info->input_crop.top * _heightIn2OutScale);
        _originalCropScalerToOutput.bottom += static_cast<int32_t>(
            outputRunKernel->resolution_info->input_crop.bottom * _heightIn2OutScale);

        _widthIn2OutScale *=
            static_cast<double>(outputRunKernel->resolution_info->input_width -
                                outputRunKernel->resolution_info->input_crop.left -
                                outputRunKernel->resolution_info->input_crop.right) /
            outputRunKernel->resolution_info->output_width;

        _heightIn2OutScale *=
            static_cast<double>(outputRunKernel->resolution_info->input_height -
                                outputRunKernel->resolution_info->input_crop.top -
                                outputRunKernel->resolution_info->input_crop.bottom) /
            outputRunKernel->resolution_info->output_height;
    }
    ...
    if (sensorMode->horizontalScalingDenominator) {
        _sensorHorizontalScaling = static_cast<double>(sensorMode->horizontalScalingNumerator) /
                                   sensorMode->horizontalScalingDenominator;
    }
    if (sensorMode->verticalScalingDenominator) {
        _sensorVerticalScaling = static_cast<double>(sensorMode->verticalScalingNumerator) /
                                 sensorMode->verticalScalingDenominator;
    }
    ...
    _originalCropInputToScaler.top = static_cast<int32_t>(
        (_originalCropInputToScaler.top - static_cast<int32_t>(_sensorVerticalCropTop)) *
        _sensorHorizontalScaling);
    _originalCropInputToScaler.bottom = static_cast<int32_t>(
        (_originalCropInputToScaler.bottom - static_cast<int32_t>(_sensorVerticalCropBottom)) *
        _sensorHorizontalScaling);
    _originalCropInputToScaler.left = static_cast<int32_t>(
        (_originalCropInputToScaler.left - static_cast<int32_t>(_sensorHorizontalCropLeft)) *
        _sensorVerticalScaling);
    _originalCropInputToScaler.right = static_cast<int32_t>(
        (_originalCropInputToScaler.right - static_cast<int32_t>(_sensorHorizontalCropRight)) *
        _sensorVerticalScaling);
}
...
StaticGraphStatus GraphResolutionConfigurator::getZoomKeyResolutionIndex(
    ...
    if (width > 0 && height > 0) {
        for (uint32_t i = 0; i < zoomKeyResolutions->numberOfZoomKeyOptions; ++i) {
            if (width > zoomKeyResolutions->zoomKeyResolutionOptions[i].width ||
                height > zoomKeyResolutions->zoomKeyResolutionOptions[i].height) {
                // This key resolution is too small, so use previous one (selected index i means use
                // previous key resolution, not this one)
                selectedIndex = i;
                break;
            }
        }
...
#endif
// This function receives a static graph and updates kernels resolution info and resolution history
// to perform the required crop and scaling for the give roi Expected changes in graph: Down scaler
// resolution info - crop & scale Up scaler resolution info - crop  & scale
//
// Up scaler resolution history
// All kerenls after upscaler - resolution history
//
// When using key resolutions:
// If previous ROI is supplied, the function will also return indication if key resolution has
// changed. A change of key resolution indicates that resolutions of entire pipe had changed until
// after the up scaler
StaticGraphStatus GraphResolutionConfigurator::updateStaticGraphConfig(
    const RegionOfInterest& roi, const RegionOfInterest& prevRoi, bool isCenteredZoom,
    bool prevIsCenteredZoom, bool& isKeyResolutionChanged) {
#if SUPPORT_KEY_RESOLUTIONS == 1
    if (_staticGraph == nullptr) {
    ...
    if (isCenteredZoom == true) {
        ...
        if (userRoi.zoomFactor == 1.0) {
            keyResIndex = 0;
        } else if (getZoomKeyResolutionIndex(zoomKeyResolutions, sensorRoi, keyResIndex) !=
                   StaticGraphStatus::SG_OK) {
            return StaticGraphStatus::SG_ERROR;
        }
    ...
    if (prevIsCenteredZoom == true) {
        ...
        if (prevUserRoi.zoomFactor == 1.0) {
            prevKeyResIndex = 0;
        } else if (getZoomKeyResolutionIndex(zoomKeyResolutions, prevSensorRoi, prevKeyResIndex) !=
                   StaticGraphStatus::SG_OK) {
            return StaticGraphStatus::SG_ERROR;
        }
    }
...
// This function translates ROI from factors (as given by user) to sensor resolution (as required by
// resolution Configurator) There are 2 modes of work - if userRoi.fromInput is true it means
// zoomFactor panFactor and tiltFactor are relative to sensor FOV if userRoi.fromInput is false it
// means zoomFactor panFactor and tiltFactor are relative to preview pin output FOV
StaticGraphStatus GraphResolutionConfigurator::getSensorRoi(const RegionOfInterest& userRoi,
                                                            SensorRoi& sensorRoi) {
    if (_staticGraph == nullptr) {
        return StaticGraphStatus::SG_ERROR;
    }
    ...
    // Calculate ROI on output, using original output since user ROI is relative to full output ROI
    // (not after some zoom was performed)
    ...
    uint32_t outputLeft =
        static_cast<uint32_t>(outputRunKernelResolution->output_width * userRoi.panFactor);
    uint32_t outputRight = static_cast<uint32_t>(outputRunKernelResolution->output_width *
                                                 (1 - userRoi.panFactor - userRoi.zoomFactor));
    uint32_t outputTop =
        static_cast<uint32_t>(outputRunKernelResolution->output_height * userRoi.tiltFactor);
    uint32_t outputBottom = static_cast<uint32_t>(outputRunKernelResolution->output_height *
                                                  (1 - userRoi.tiltFactor - userRoi.zoomFactor));
    ...
    uint32_t inputWidth =
        static_cast<uint32_t>((outputRunKernel->resolution_history->input_width -
                               _sensorHorizontalCropLeft - _sensorHorizontalCropRight) *
                              _sensorHorizontalScaling);
    uint32_t inputHeight =
        static_cast<uint32_t>((outputRunKernel->resolution_history->input_height -
                               _sensorVerticalCropTop - _sensorVerticalCropBottom) *
                              _sensorVerticalScaling);

    // Translate to ROI on input
    // We round down to make sure crops are not less than original crop
    // We add to sensor ROI the cropping done after scalers since it will be cropped in zoom
    // configurations as well.
    sensorRoi.width = GRA_ROUND_DOWN(
        static_cast<uint32_t>(outputWidth * _widthIn2OutScale + _originalCropScalerToOutput.left +
                              _originalCropScalerToOutput.right),
        2);
    sensorRoi.height = GRA_ROUND_DOWN(
        static_cast<uint32_t>(outputHeight * _heightIn2OutScale + _originalCropScalerToOutput.top +
                              _originalCropScalerToOutput.bottom),
        2);
    sensorRoi.cropLeft = GRA_ROUND_UP(
        static_cast<uint32_t>((outputLeft * _widthIn2OutScale) + _originalCropInputToScaler.left),
        2);
    sensorRoi.cropRight = inputWidth - sensorRoi.width - sensorRoi.cropLeft;
    sensorRoi.cropTop = GRA_ROUND_UP(
        static_cast<uint32_t>((outputTop * _heightIn2OutScale) + _originalCropInputToScaler.top),
        2);
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelOfScalers(SensorRoi& roi) {
    ...
    downscalerCropHistory.left =
        static_cast<int32_t>((downscalerRunKernel->resolution_history->input_crop.left -
                              static_cast<int32_t>(_sensorHorizontalCropLeft)) *
                             _sensorHorizontalScaling);
    downscalerCropHistory.right =
        static_cast<int32_t>((downscalerRunKernel->resolution_history->input_crop.right -
                              static_cast<int32_t>(_sensorHorizontalCropRight)) *
                             _sensorHorizontalScaling);
    downscalerCropHistory.top =
        static_cast<int32_t>((downscalerRunKernel->resolution_history->input_crop.top -
                              static_cast<int32_t>(_sensorVerticalCropTop)) *
                             _sensorVerticalScaling);
    downscalerCropHistory.bottom =
        static_cast<int32_t>((downscalerRunKernel->resolution_history->input_crop.bottom -
                              static_cast<int32_t>(_sensorVerticalCropBottom)) *
                             _sensorVerticalScaling);
    ...
    if (roi.width >= outputWidth) {
        ...
        if (updateRunKernelDownScaler(downscalerRunKernel, roi, inputWidth, inputHeight,
                                      ...
                                      &downscalerCropHistory) != StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }

        updateRunKernelPassThrough(upscalerRunKernel, outputWidth, outputHeight);

        // When downscaling, cropper is not a part of dynamic scaling, even if it was a part of
        ...
        // Update upscaler info, according constraints. Returns the expected input width and height
        // for upscaler.
        ...
        if (updateRunKernelUpScaler(upscalerRunKernel, roi.width, roi.height, outputWidth,
                                    outputHeight, upscalerActualInputWidth,
                                    upscalerActualInputHeight, upscalerActualOutputWidth,
                                    upscalerActualOutputHeight) != StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
        ...
        // Now that we're done, if downscaler is not doing any scaling, it is better to let it be
        // bypassed and move the cropping to upscaler
        updateCroppingScaler(downscalerRunKernel, upscalerRunKernel);

        // Update ESPA crop if required
        updateRunKernelFinalCropper(cropperRunKernel, upscalerActualOutputWidth,
                                    upscalerActualOutputHeight, outputWidth, outputHeight);
    }

    // Update resolution histories according to decisions made above
    if (updateRunKernelResolutionHistory(upscalerRunKernel, downscalerRunKernel) !=
        StaticGraphStatus::SG_OK) {
        ret = StaticGraphStatus::SG_ERROR;
    }

    if (updateRunKernelResolutionHistory(cropperRunKernel, upscalerRunKernel) !=
        StaticGraphStatus::SG_OK) {
        ret = StaticGraphStatus::SG_ERROR;
    }
    ...
    for (auto& runKernelForUpdate : _kernelsForUpdate) {
        // We update all histories according to upscaler... ignoring any cropping from now on, even
        // if we configured ESPA cropper.
        StaticGraphRunKernel* runKernelPtr = getRunKernel(runKernelForUpdate);
        if (updateRunKernelResolutionHistory(runKernelPtr, upscalerRunKernel, false) !=
            StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
    }
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelDownScaler(
    StaticGraphRunKernel* runKernel, SensorRoi& roi, uint32_t inputWidth, uint32_t inputHeight,
    ...
    // We assume that originally DS was configured to crop all the padding in its resolution
    // history. Otherwise - we will need to save original DS crop (but for each key resolution...)
    // roi crops were rounded, while original crop may still be odd numbers. We need to ignore 1
    // pixel diffs
    ...
    // If we are very close to key resolution sizes, and sensor BYR order is not GRBG, we may have
    // small negative crops here
...
StaticGraphStatus GraphResolutionConfigurator::adjustDownscalerCrop(
    StaticGraphKernelRes* scalerResInfo) {
    if (scalerResInfo->input_width - scalerResInfo->input_crop.left -
            scalerResInfo->input_crop.right <
        scalerResInfo->output_width) {
        uint32_t extraPixels = scalerResInfo->output_width -
                               (scalerResInfo->input_width - scalerResInfo->input_crop.left -
                                scalerResInfo->input_crop.right);
    ...
    if (scalerResInfo->input_height - scalerResInfo->input_crop.top -
            scalerResInfo->input_crop.bottom <
        scalerResInfo->output_height) {
        uint32_t extraPixels = scalerResInfo->output_height -
                               (scalerResInfo->input_height - scalerResInfo->input_crop.top -
                                scalerResInfo->input_crop.bottom);
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelUpScaler(
    StaticGraphRunKernel* runKernel, uint32_t inputWidth, uint32_t inputHeight,
    uint32_t outputWidth, uint32_t outputHeight, uint32_t& upscalerActualInputWidth,
    uint32_t& upscalerActualInputHeight, uint32_t& upscalerActualOutputWidth,
    uint32_t& upscalerActualOutputHeight) {
    ...
    // This is usually better when US output is not regular (and mp/dp cropping is used) and/or DS
    // input is irregular (and ESPA is fixing A/R in original settings) HSD 15016169206 and
    // 15017041003 are 2 examples
    uint32_t newOutputWidth =
        outputWidth + _originalCropOfFinalCropper.left + _originalCropOfFinalCropper.right;
    uint32_t newOutputHeight =
        outputHeight + _originalCropOfFinalCropper.top + _originalCropOfFinalCropper.bottom;
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelPassThrough(
    StaticGraphRunKernel* runKernel, uint32_t width, uint32_t height) {
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelFinalCropper(
    StaticGraphRunKernel* runKernel, uint32_t inputWidth, uint32_t inputHeight,
...
StaticGraphStatus GraphResolutionConfigurator::updateCroppingScaler(
    StaticGraphRunKernel* downscalerRunKernel, StaticGraphRunKernel* upscalerRunKernel) {
    // Is DS performing DS?
    int32_t widthAfterCtop = downscalerRunKernel->resolution_info->input_width -
                             downscalerRunKernel->resolution_info->input_crop.left -
    ...
    int32_t heightAfterCtop = downscalerRunKernel->resolution_info->input_height -
                              downscalerRunKernel->resolution_info->input_crop.top -
                              downscalerRunKernel->resolution_info->input_crop.bottom;
    ...
    if (widthAfterCtop == downscalerRunKernel->resolution_info->output_width &&
        ...
        upscalerRunKernel->resolution_info->input_crop.left =
            downscalerRunKernel->resolution_info->input_crop.left;
        upscalerRunKernel->resolution_info->input_crop.right =
            downscalerRunKernel->resolution_info->input_crop.right;
        upscalerRunKernel->resolution_info->input_crop.top =
            downscalerRunKernel->resolution_info->input_crop.top;
        upscalerRunKernel->resolution_info->input_crop.bottom =
            downscalerRunKernel->resolution_info->input_crop.bottom;

        upscalerRunKernel->resolution_info->input_width =
            downscalerRunKernel->resolution_info->input_width;
        upscalerRunKernel->resolution_info->input_height =
            downscalerRunKernel->resolution_info->input_height;
        ...
        downscalerRunKernel->resolution_info->output_width =
            downscalerRunKernel->resolution_info->input_width;
        downscalerRunKernel->resolution_info->output_height =
            downscalerRunKernel->resolution_info->input_height;
    }
...
StaticGraphStatus GraphResolutionConfigurator::updateRunKernelResolutionHistory(
    StaticGraphRunKernel* runKernel, StaticGraphRunKernel* prevRunKernel, bool updateResolution) {
    runKernel->resolution_history->input_crop.left =
        prevRunKernel->resolution_history->input_crop.left +
        static_cast<uint32_t>(prevRunKernel->resolution_info->input_crop.left /
                              _sensorHorizontalScaling);
    runKernel->resolution_history->input_crop.right =
        prevRunKernel->resolution_history->input_crop.right +
        static_cast<uint32_t>(prevRunKernel->resolution_info->input_crop.right /
                              _sensorHorizontalScaling);
    runKernel->resolution_history->input_crop.top =
        prevRunKernel->resolution_history->input_crop.top +
        static_cast<uint32_t>(prevRunKernel->resolution_info->input_crop.top /
                              _sensorVerticalScaling);
    runKernel->resolution_history->input_crop.bottom =
        prevRunKernel->resolution_history->input_crop.bottom +
        static_cast<uint32_t>(prevRunKernel->resolution_info->input_crop.bottom /
                              _sensorVerticalScaling);
...
StaticGraphStatus GraphResolutionConfigurator::findRunKernel(uint32_t kernelUuid,
...
StaticGraphStatus GraphResolutionConfigurator::initRunKernelCoord(
    GraphResolutionConfiguratorKernelRole role, RunKernelCoords& coord) {
...
StaticGraphStatus GraphResolutionConfigurator::initOutputRunKernelCoord(RunKernelCoords& coord) {
    ...
    // Find output device
    auto kernelUuid =
        GraphResolutionConfiguratorHelper::getRunKernelUuidOfOutput(hwSink, graphId, links);
    return findRunKernel(kernelUuid, coord);
}
...
StaticGraphStatus GraphResolutionConfigurator::getInputRoiForOutput(const ResolutionRoi& roi,
                                                                    HwSink hwSink,
                                                                    SensorRoi& sensorRoi) {
    if (_staticGraph == nullptr) {
        return StaticGraphStatus::SG_ERROR;
    ...
    // We want to calculate the scaling ratio without taking any cropping into consideration
    double widthIn2OutScale =
        static_cast<double>(outputRunKernel->resolution_history->input_width -
                            outputRunKernel->resolution_history->input_crop.left -
                            outputRunKernel->resolution_history->input_crop.right) /
        outputRunKernel->resolution_history->output_width;

    double heightIn2OutScale =
        static_cast<double>(outputRunKernel->resolution_history->input_height -
                            outputRunKernel->resolution_history->input_crop.top -
                            outputRunKernel->resolution_history->input_crop.bottom) /
        outputRunKernel->resolution_history->output_height;
    ...
    if (outputRunKernel->resolution_info != nullptr) {
        ...
        // Translate crop to sensor units, w/out this kernel's scaling since input crop is done
        // before scaling.
    ...
    sensorRoi.cropLeft =
        GRA_ROUND_UP(static_cast<uint32_t>((roi.left * widthIn2OutScale) +
                                           ((outputCropHist.left - _sensorHorizontalCropLeft) *
                                            _sensorHorizontalScaling) +
                                           outputCrop.left),
                     2);
    sensorRoi.cropRight =
        GRA_ROUND_UP(static_cast<uint32_t>((roi.right * widthIn2OutScale) +
                                           ((outputCropHist.right - _sensorHorizontalCropRight) *
                                            _sensorHorizontalScaling) +
                                           outputCrop.right),
                     2);
    sensorRoi.cropTop =
        GRA_ROUND_UP(static_cast<uint32_t>(
                         (roi.top * heightIn2OutScale) +
                         ((outputCropHist.top - _sensorVerticalCropTop) * _sensorVerticalScaling) +
                         outputCrop.top),
                     2);
    sensorRoi.cropBottom =
        GRA_ROUND_UP(static_cast<uint32_t>((roi.bottom * heightIn2OutScale) +
                                           ((outputCropHist.bottom - _sensorVerticalCropBottom) *
                                            _sensorVerticalScaling) +
                                           outputCrop.bottom),
                     2);
...
StaticGraphStatus GraphResolutionConfigurator::getStatsRoiFromSensorRoi(const SensorRoi& sensorRoi,
    ...
    RunKernelCoords coord;
    auto kernelUuid = GraphResolutionConfiguratorHelper::getRunKernelUuidOfOutput(HwSink::AeOutSink,
                                                                                  graphId, links);
    ...
    double widthIn2OutScale =
        static_cast<double>(aeRunKernel->resolution_history->output_width) /
        (aeRunKernel->resolution_history->input_width - aeCropHist.left - aeCropHist.right);

    double heightIn2OutScale =
        static_cast<double>(aeRunKernel->resolution_history->output_height) /
    ...
    statsRoi.left = GRA_ROUND_UP(
        static_cast<uint32_t>((sensorRoi.cropLeft * widthIn2OutScale) + aeCropHist.left), 2);
    statsRoi.right = GRA_ROUND_UP(
        static_cast<uint32_t>((sensorRoi.cropRight * widthIn2OutScale) + aeCropHist.right), 2);
    statsRoi.top = GRA_ROUND_UP(
        static_cast<uint32_t>((sensorRoi.cropTop * heightIn2OutScale) + aeCropHist.top), 2);
    statsRoi.bottom = GRA_ROUND_UP(
        static_cast<uint32_t>((sensorRoi.cropBottom * heightIn2OutScale) + aeCropHist.bottom), 2);
...
StaticGraphStatus GraphResolutionConfigurator::undoSensorCropandScale(SensorRoi& sensor_roi) {
    sensor_roi.width =
        GRA_ROUND_UP(static_cast<uint32_t>(sensor_roi.width / _sensorHorizontalScaling), 2);
    sensor_roi.height =
        GRA_ROUND_UP(static_cast<uint32_t>(sensor_roi.height / _sensorVerticalScaling), 2);
    sensor_roi.cropLeft =
        GRA_ROUND_UP(static_cast<uint32_t>((sensor_roi.cropLeft / _sensorHorizontalScaling) +
                                           _sensorHorizontalCropLeft),
                     2);
    sensor_roi.cropRight =
        GRA_ROUND_UP(static_cast<uint32_t>((sensor_roi.cropRight / _sensorHorizontalScaling) +
                                           _sensorHorizontalCropRight),
                     2);
    sensor_roi.cropTop =
        GRA_ROUND_UP(static_cast<uint32_t>((sensor_roi.cropTop / _sensorVerticalScaling) +
                                           _sensorVerticalCropTop),
                     2);
    sensor_roi.cropBottom =
        GRA_ROUND_UP(static_cast<uint32_t>((sensor_roi.cropBottom / _sensorVerticalScaling) +
                                           _sensorVerticalCropBottom),
                     2);

    return StaticGraphStatus::SG_OK;
}

StaticGraphStatus GraphResolutionConfigurator::sensorCropOrScaleExist(
    bool& sensor_crop_or_scale_exist) {
    sensor_crop_or_scale_exist = false;
    if ((abs(_sensorHorizontalScaling - 1.0F) < 0.01F) ||
        (abs(_sensorVerticalScaling - 1.0F) < 0.01F) || (_sensorHorizontalCropLeft > 0) ||
        (_sensorHorizontalCropRight > 0) || (_sensorVerticalCropTop > 0) ||
...
Ipu8GraphResolutionConfigurator::Ipu8GraphResolutionConfigurator(IStaticGraphConfig* staticGraph)
    ...
    if (initRunKernel(GraphResolutionConfiguratorKernelRole::DownScaler, _downscalerRunKernel) !=
            StaticGraphStatus::SG_OK ||
        initRunKernel(GraphResolutionConfiguratorKernelRole::UpScaler, _upscalerRunKernel) !=
            StaticGraphStatus::SG_OK ||
        initRunKernel(GraphResolutionConfiguratorKernelRole::EspaCropper, _cropperRunKernel) !=
            StaticGraphStatus::SG_OK ||
    ...
    _widthIn2OutScale =
        static_cast<double>(_outputRunKernel->resolution_history->input_width -
                            _outputRunKernel->resolution_history->input_crop.left -
                            _outputRunKernel->resolution_history->input_crop.right) /
        _outputRunKernel->resolution_history->output_width;

    _heightIn2OutScale =
        static_cast<double>(_outputRunKernel->resolution_history->input_height -
                            _outputRunKernel->resolution_history->input_crop.top -
                            _outputRunKernel->resolution_history->input_crop.bottom) /
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::initRunKernel(
    GraphResolutionConfiguratorKernelRole role, StaticGraphRunKernel*& runKernel) {
    uint32_t kernelUuid = GraphResolutionConfiguratorHelper::getRunKernelUuid(role);
    return initRunKernel(kernelUuid, runKernel);
}

StaticGraphStatus Ipu8GraphResolutionConfigurator::initRunKernel(uint32_t kernelUuid,
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::initKernelsForUpdate() {
    ...
    GraphResolutionConfiguratorHelper::getRunKernelUuidForResHistoryUpdate(
        kernelUuids, _cropperRunKernel->kernel_uuid);
    ...
    GraphResolutionConfiguratorHelper::getRunKernelUuidForResHistoryUpdate(
        kernelUuids, _upscalerRunKernel->kernel_uuid);
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateStaticGraphConfig(
    const RegionOfInterest& roi, bool isCenteredZoom) {
    if (_staticGraph == nullptr) {
        return StaticGraphStatus::SG_ERROR;
    }
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::getDownscalerInputRoi(
    const RegionOfInterest& userRoi, ResolutionRoi& downscalerInputRoi) {
    if (_staticGraph == nullptr) {
        return StaticGraphStatus::SG_ERROR;
    }
    ...
    pipeInputRoi.left =
        static_cast<uint32_t>(((outputLeft + _originalCropOfOutput.left) * _widthIn2OutScale) +
                              _originaHistoryOfOutput.left);
    pipeInputRoi.right =
        static_cast<uint32_t>(((outputRight + _originalCropOfOutput.right) * _widthIn2OutScale) +
                              _originaHistoryOfOutput.right);
    pipeInputRoi.top =
        static_cast<uint32_t>(((outputTop + _originalCropOfOutput.top) * _heightIn2OutScale) +
                              _originaHistoryOfOutput.top);
    pipeInputRoi.bottom =
        static_cast<uint32_t>(((outputBottom + _originalCropOfOutput.bottom) * _heightIn2OutScale) +
                              _originaHistoryOfOutput.bottom);

    // Translate ROI on input to ROI as input to downscaler
    double scaleWidth =
        static_cast<double>(_downscalerRunKernel->resolution_history->input_width -
    ...
    double scaleHeight =
        static_cast<double>(_downscalerRunKernel->resolution_history->input_height -
                            _downscalerRunKernel->resolution_history->input_crop.top -
                            _downscalerRunKernel->resolution_history->input_crop.bottom) /
        _downscalerRunKernel->resolution_history->output_height;

    downscalerInputRoi.left = GRA_ROUND_UP(
        static_cast<uint32_t>(
            (pipeInputRoi.left - _downscalerRunKernel->resolution_history->input_crop.left) /
            scaleWidth),
        2);
    downscalerInputRoi.right = GRA_ROUND_UP(
        static_cast<uint32_t>(
            (pipeInputRoi.right - _downscalerRunKernel->resolution_history->input_crop.right) /
            scaleWidth),
        2);
    downscalerInputRoi.top = GRA_ROUND_UP(
        static_cast<uint32_t>(
            (pipeInputRoi.top - _downscalerRunKernel->resolution_history->input_crop.top) /
            scaleHeight),
        2);
    downscalerInputRoi.bottom = GRA_ROUND_UP(
        static_cast<uint32_t>(
            (pipeInputRoi.bottom - _downscalerRunKernel->resolution_history->input_crop.bottom) /
            scaleHeight),
        2);

    downscalerInputRoi.width = _downscalerRunKernel->resolution_history->output_width -
                               downscalerInputRoi.left - downscalerInputRoi.right;
    downscalerInputRoi.height = _downscalerRunKernel->resolution_history->output_height -
                                downscalerInputRoi.top - downscalerInputRoi.bottom;
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateRunKernelOfScalers(ResolutionRoi& roi) {
    ...
    StaticGraphKernelResCrop downscalerCropHist =
        _downscalerRunKernel->resolution_history->input_crop;

    // If ROI is larger than scaler's output resolution - we downscale
    if (roi.width >= outputWidth) {
        // Only down scaler is active
        if (updateRunKernelDownScaler(_downscalerRunKernel, roi, outputWidth, outputHeight) !=
            StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }

        // Pass-through. Must call cropper function in order to update the system API.
        StaticGraphKernelResCrop EmptyCrop = {0, 0, 0, 0};
        if (updateRunKernelCropper(_cropperRunKernel, outputWidth, outputHeight, outputWidth,
                                   outputHeight, EmptyCrop) != StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
        ...
        if (updateRunKernelCropper(_cropperRunKernel, inputWidth, inputHeight, outputWidth,
                                   outputHeight, downscalerCropHist) != StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }

        // Configure upscaler to crop from output resolution to ROI
        if (updateRunKernelUpScaler(_upscalerRunKernel, roi,
                                    _cropperRunKernel->resolution_info->input_crop, outputWidth,
                                    outputHeight) != StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
    }

    // Update resolution histories according to decisions made above
    if (updateRunKernelResolutionHistory(_cropperRunKernel, _downscalerRunKernel) !=
        StaticGraphStatus::SG_OK) {
        ret = StaticGraphStatus::SG_ERROR;
    }

    if (updateRunKernelResolutionHistory(_upscalerRunKernel, _cropperRunKernel) !=
        StaticGraphStatus::SG_OK) {
        ret = StaticGraphStatus::SG_ERROR;
    }
    ...
    for (auto& runKernelForUpdate : _kernelsForUpdateAfterCropper) {
        ...
        if (updateRunKernelResolutionHistory(runKernelForUpdate, _cropperRunKernel, false) !=
            StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
    }
    ...
    for (auto& runKernelForUpdate : _kernelsForUpdateAfterUpscaler) {
        // We update all histories according to our changes... assuming no more cropping from now on
        if (updateRunKernelResolutionHistory(runKernelForUpdate, _upscalerRunKernel, false) !=
            StaticGraphStatus::SG_OK) {
            ret = StaticGraphStatus::SG_ERROR;
        }
    }
    ...
    // updateKernelFragments(_downscalerRunKernel, downscalerFragments, fragments);
    // updateKernelFragments(_upscalerRunKernel, upscalerFragments, fragments);
    // updateKernelFragments(_cropperRunKernel, cropperFragments, fragments);
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateRunKernelDownScaler(
    StaticGraphRunKernel* runKernel, ResolutionRoi& roi, uint32_t outputWidth,
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateRunKernelCropper(
    StaticGraphRunKernel* runKernel, uint32_t inputWidth, uint32_t inputHeight,
    ...
    // Configure to crop the required amount. First try to use the original DS cropping (Remove
    // padding)
    int32_t totalHorizontalCrop = inputWidth - outputWidth;

    int32_t originalDsPadding = downscalerCropHist.left < 0 ? -downscalerCropHist.left : 0;
    ...
    originalDsPadding = downscalerCropHist.right < 0 ? -downscalerCropHist.right : 0;
    if (totalHorizontalCrop >= originalDsPadding && originalDsPadding > 0) {
    ...
    runKernel->resolution_info->input_crop.left +=
        GRA_ROUND_DOWN(static_cast<uint32_t>(static_cast<double>(totalHorizontalCrop)) / 2, 2);
    runKernel->resolution_info->input_crop.right +=
        GRA_ROUND_UP(static_cast<uint32_t>(static_cast<double>(totalHorizontalCrop)) / 2, 2);

    // Configure to crop the required amount. First try to use the original DS cropping (Remove
    // padding)
    int32_t totalVerticalCrop = inputHeight - outputHeight;

    originalDsPadding = downscalerCropHist.top < 0 ? -downscalerCropHist.top : 0;
    ...
    originalDsPadding = downscalerCropHist.bottom < 0 ? -downscalerCropHist.bottom : 0;
    if (totalVerticalCrop >= originalDsPadding && originalDsPadding > 0) {
    ...
    // Now crop symmetrically to TNR size if any more cropping is required
    runKernel->resolution_info->input_crop.top +=
        GRA_ROUND_DOWN(static_cast<uint32_t>(static_cast<double>(totalVerticalCrop)) / 2, 2);
    runKernel->resolution_info->input_crop.bottom +=
        GRA_ROUND_UP(static_cast<uint32_t>(static_cast<double>(totalVerticalCrop)) / 2, 2);

#ifdef STATIC_GRAPH_USE_IA_LEGACY_TYPES
    // Update the left crop in striping system api. Currently assuming one stripe
    if (runKernel->system_api.size != ((GRA_ROUND_UP(sizeof(SystemApiRecordHeader), 4)) +
                                       (sizeof(StaticGraphKernelSystemApiIoBuffer1_4)))) {
        // TODO log error
        return StaticGraphStatus::SG_ERROR;
    }
#endif
    ...
    if (systemApiHeader->systemApiUuid !=
        GraphResolutionConfiguratorHelper::getRunKernelIoBufferSystemApiUuid()) {
        // TODO log error
        return StaticGraphStatus::SG_ERROR;
    }

    StaticGraphKernelSystemApiIoBuffer1_4* systemApi =
        reinterpret_cast<StaticGraphKernelSystemApiIoBuffer1_4*>(
            static_cast<int8_t*>(runKernel->system_api.data) +
            GRA_ROUND_UP(sizeof(SystemApiRecordHeader), 4));
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateRunKernelUpScaler(
    StaticGraphRunKernel* runKernel, ResolutionRoi& roi,
    StaticGraphKernelResCrop& cropperKernelCrop, uint32_t outputWidth, uint32_t outputHeight) {
    StaticGraphStatus ret = StaticGraphStatus::SG_OK;
    ...
    uint32_t deltaW = GRA_ROUND_DOWN((totalCropW - actualCropW) / 2, 2);
    uint32_t deltaH = GRA_ROUND_DOWN((totalCropH - actualCropH) / 2, 2);

    runKernel->resolution_info->input_crop.left = roi.left - cropperKernelCrop.left - deltaW;
    runKernel->resolution_info->input_crop.right =
        actualCropW - runKernel->resolution_info->input_crop.left;
    runKernel->resolution_info->input_crop.top = roi.top - cropperKernelCrop.top - deltaH;
    runKernel->resolution_info->input_crop.bottom =
        actualCropH - runKernel->resolution_info->input_crop.top;

    return ret;
}

StaticGraphFragmentDesc* Ipu8GraphResolutionConfigurator::getKernelFragments(
    RunKernelCoords& coord) {
    GraphTopology* graphTopology = nullptr;
    StaticGraphStatus status = _staticGraph->getGraphTopology(&graphTopology);
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::updateKernelFragments(
    StaticGraphRunKernel* runKernel, StaticGraphFragmentDesc* fragmentsDesc, uint32_t fragments) {
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::SanityCheck() {
    ...
    if (_downscalerRunKernel->resolution_info->input_width !=
            _downscalerRunKernel->resolution_history->output_width ||
        _downscalerRunKernel->resolution_info->input_height !=
            _downscalerRunKernel->resolution_history->output_height ||

        _upscalerRunKernel->resolution_info->input_width !=
            _upscalerRunKernel->resolution_history->output_width ||
        _upscalerRunKernel->resolution_info->input_height !=
            _upscalerRunKernel->resolution_history->output_height ||

        _cropperRunKernel->resolution_info->input_width !=
            _cropperRunKernel->resolution_history->output_width ||
        _cropperRunKernel->resolution_info->input_height !=
            _cropperRunKernel->resolution_history->output_height) {
        return StaticGraphStatus::SG_ERROR;
    }

    // Resolution consistency
    if (_downscalerRunKernel->resolution_info->output_width !=
            _cropperRunKernel->resolution_info->input_width ||
        _downscalerRunKernel->resolution_info->output_height !=
            _cropperRunKernel->resolution_info->input_height ||

        _cropperRunKernel->resolution_info->output_width !=
            _upscalerRunKernel->resolution_info->input_width ||
        _cropperRunKernel->resolution_info->output_height !=
            _upscalerRunKernel->resolution_info->input_height) {
        return StaticGraphStatus::SG_ERROR;
    }
    ...
    if (SanityCheckCrop(&_downscalerRunKernel->resolution_info->input_crop) ==
            StaticGraphStatus::SG_ERROR ||
        SanityCheckCrop(&_upscalerRunKernel->resolution_info->input_crop) ==
            StaticGraphStatus::SG_ERROR ||
        SanityCheckCrop(&_cropperRunKernel->resolution_info->input_crop) ==
            StaticGraphStatus::SG_ERROR) {
        return StaticGraphStatus::SG_ERROR;
    }

    // Make sure DS is actually down scaling
    int32_t widthAfterCrop = _downscalerRunKernel->resolution_info->input_width -
                             _downscalerRunKernel->resolution_info->input_crop.left -
                             _downscalerRunKernel->resolution_info->input_crop.right;
    int32_t heightAfterCrop = _downscalerRunKernel->resolution_info->input_height -
                              _downscalerRunKernel->resolution_info->input_crop.top -
                              _downscalerRunKernel->resolution_info->input_crop.bottom;

    if (widthAfterCrop < 0 ||
        widthAfterCrop < _downscalerRunKernel->resolution_info->output_width ||
        heightAfterCrop < 0 ||
        heightAfterCrop < _downscalerRunKernel->resolution_info->output_height ||
        static_cast<double>(widthAfterCrop) / _downscalerRunKernel->resolution_info->output_width >
            16) {
        return StaticGraphStatus::SG_ERROR;
    }

    // Make sure US is actually up scaling
    widthAfterCrop = _upscalerRunKernel->resolution_info->input_width -
                     _upscalerRunKernel->resolution_info->input_crop.left -
                     _upscalerRunKernel->resolution_info->input_crop.right;
    heightAfterCrop = _upscalerRunKernel->resolution_info->input_height -
                      _upscalerRunKernel->resolution_info->input_crop.top -
                      _upscalerRunKernel->resolution_info->input_crop.bottom;

    if (widthAfterCrop < 0 || widthAfterCrop > _upscalerRunKernel->resolution_info->output_width ||
        heightAfterCrop < 0 ||
        heightAfterCrop > _upscalerRunKernel->resolution_info->output_height ||
        static_cast<double>(_upscalerRunKernel->resolution_history->output_width) / widthAfterCrop >
            16) {
        return StaticGraphStatus::SG_ERROR;
    }
    ...
    widthAfterCrop = _cropperRunKernel->resolution_info->input_width -
                     _cropperRunKernel->resolution_info->input_crop.left -
                     _cropperRunKernel->resolution_info->input_crop.right;
    heightAfterCrop = _cropperRunKernel->resolution_info->input_height -
                      _cropperRunKernel->resolution_info->input_crop.top -
                      _cropperRunKernel->resolution_info->input_crop.bottom;

    if (widthAfterCrop < 0 || widthAfterCrop != _cropperRunKernel->resolution_info->output_width ||
        heightAfterCrop < 0 ||
        heightAfterCrop != _cropperRunKernel->resolution_info->output_height) {
        return StaticGraphStatus::SG_ERROR;
    }
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::SanityCheckCrop(StaticGraphKernelResCrop* crop) {
    if (crop->top < 0 || crop->bottom < 0 || crop->left < 0 || crop->right < 0 || crop->top & 1 ||
        crop->bottom & 1 || crop->left & 1 || crop->right & 1) {
        return StaticGraphStatus::SG_ERROR;
    }
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::getInputRoiForOutput(const ResolutionRoi& roi,
                                                                        HwSink hwSink,
                                                                        SensorRoi& sensorRoi) {
    if (_staticGraph == nullptr) {
        return StaticGraphStatus::SG_ERROR;
...
StaticGraphStatus Ipu8GraphResolutionConfigurator::getStatsRoiFromSensorRoi(
    const SensorRoi& sensorRoi, ResolutionRoi& statsRoi) {
    int32_t graphId;
    _staticGraph->getGraphId(&graphId);

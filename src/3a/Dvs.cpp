...
int Dvs::configCcaDvsData(const ConfigMode configMode, cca::cca_init_params *params) {
    ...
    std::shared_ptr<GraphConfig> gc =
        CameraContext::getInstance(mCameraId)->getGraphConfig(configMode);
...
void Dvs::handleEvent(EventData eventData) {
    ...
    auto dataContext = cameraContext->getDataContextBySeq(eventData.data.statsReady.sequence);

    const camera_zoom_region_t& ptzRegion = dataContext->zoomRegion;
    // Run DVS

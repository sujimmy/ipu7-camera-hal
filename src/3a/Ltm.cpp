...
const aiq_parameter_t* Ltm::getAiqInputParams(int64_t sequence) {
    ...
    auto dataContext = cameraContext->getDataContextBySeq(ltmSequence);
    CheckAndLogError(!dataContext, nullptr, "%s, Failed to get dataContext", __func__);

    return &dataContext->mAiqParams;
}
...
int Ltm::handleSisLtm(const std::shared_ptr<CameraBuffer>& cameraBuffer) {
    ...
    const aiq_parameter_t* aiqInput = getAiqInputParams(sequence);
    CheckAndLogError(!aiqInput, UNKNOWN_ERROR, "%s, Failed to get aiqInput", __func__);

    mLtmParams[mInputParamIndex]->ltmParams.ev_shift = aiqInput->evShift;
    mLtmParams[mInputParamIndex]->ltmParams.ltm_strength_manual = aiqInput->ltmStrength;
    mLtmParams[mInputParamIndex]->ltmParams.frame_width = aiqInput->resolution.width;
    mLtmParams[mInputParamIndex]->ltmParams.frame_height = aiqInput->resolution.height;

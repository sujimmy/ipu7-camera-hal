...
EvcpParam EvcpManager::getParamFromExp(int64_t sequence) {
    EvcpParam earam = {};

    auto cameraContext = CameraContext::getInstance(mCameraId);
    auto dataContext = cameraContext->getDataContextBySeq(sequence);

    uint8_t bcMode = dataContext->mEvcpParams.bcMode;
    uint8_t ffMode = dataContext->mEvcpParams.ffMode;
    int w = dataContext->mEvcpParams.brWidth;
    int h = dataContext->mEvcpParams.brHeight;
    int bgFD = dataContext->mEvcpParams.brBgFd;

    earam.evcpEccEnabled = dataContext->mEvcpParams.eccMode;
...
void EvcpManager::prepare4Param(int64_t sequence) {
    EvcpParam curParam = getEvcpParam();
    EvcpParam expParam = getParamFromExp(sequence);

    if (!evcpParamCmp(&curParam, &expParam)) updateEvcpParam(expParam);
}

void EvcpManager::runEvcp(const camera_buffer_t& buffer, int64_t sequence) {
    prepare4Param(sequence);

...
void EXIFMaker::setSensorAeConfig(const AiqResult* aiqResult, const DataContext* dataContext) {
    LOG1("@%s", __func__);

    int64_t expTime = aiqResult->mAeResults.exposures[0].exposure[0].exposure_time_us;
    if (expTime > 0) {
    ...
    icamera::camera_rational_t aeCompensationStep = dataContext->mAiqParams.evStep;
    float stepEv = static_cast<float>(aeCompensationStep.numerator) /
                       aeCompensationStep.denominator;

    int32_t ev = dataContext->mAiqParams.evSetting;
    float evBias = static_cast<float>(ev) * stepEv;
    ...
    mExifAttributes.iso_speed_rating = aiqResult->mAeResults.exposures[0].exposure[0].iso;
...
void EXIFMaker::saveMakernote(int cameraId, uint64_t timestamp) {
    unsigned int size = 0;

    PlatformData::acquireMakernoteData(cameraId, timestamp, mMakernoteSection, size);
    if (size > 0)
        mExifAttributes.makerNoteDataSize = size;
}

void EXIFMaker::updateSensorInfo(const DataContext* dataContext, int cameraId) {
    float focal = dataContext->mJpegParams.focalLength;
    if (focal < EPSILON) {
        // Focal length is not supported, set to default value
        const std::string str = "lens.info.availableFocalLengths";
        auto vFocalLength = icamera::PlatformData::getFloatStaticMetadata(cameraId, str);
        if (vFocalLength.size() >= 1) {
            focal = vFocalLength[0];
        }
    }

    LOG2("focal length is %f", focal);
    mExifAttributes.focal_length.num = focal * mExifAttributes.focal_length.den;
    float aperture = dataContext->mJpegParams.aperture;
